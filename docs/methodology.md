...
The map renders these distinctly so a guess never looks authoritative: `ok` countries
get a solid fill, `approx` (and `disputed`) get a diagonal hatch over their score
color, and `unknown` gets a cross-hatched neutral gray. The tooltip shows the status,
the source used (which office's party, alignment data vs. ideology heuristic), and the
date of the Wikidata snapshot.
...
## History Snapshots and Backfill

The timeline on the map is built from an append-only snapshot archive:

- `data/history/YYYY-MM-DD.json` — one snapshot per date, scored countries only.
  The weekly pipeline appends a new snapshot when the data actually changed;
  existing files are never rewritten.
- `public/data/history.min.json` — all snapshots aggregated for the frontend.

Historical snapshots (source `backfill`, generated by `scripts/backfill_history.ts`)
are reconstructed from Wikidata office-holder records: head-of-government (P6)
statements carry start/end qualifiers (P580/P582), as do party membership (P102)
statements, so for each January 1st we can determine who governed and for which
party. Important honesty limits:

- Only the head-of-government strategy is reconstructed — no head-of-state fallback
  and no collective executives, so e.g. Switzerland is absent from backfilled years.
- A party's alignment/ideology is whatever Wikidata records *today*; projecting it
  onto the past ignores party drift. Every backfilled entry is therefore capped at
  status `approx`, and the tooltip labels these snapshots as reconstructed.
- Coverage thins out going back in time (Wikidata has dated statements for ~112
  countries in 2026 but only a handful in 1946) — absence of data is shown as
