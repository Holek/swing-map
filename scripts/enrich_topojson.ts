#!/usr/bin/env tsx
/**
 * Enrich TopoJSON with ISO3 codes
 * Maps numeric ISO codes to ISO3 alpha codes
 */
...
// Mapping of numeric ISO codes to ISO3 alpha codes
...
async function enrichTopojson() {
  const topoPath = path.join(__dirname, "..", "public", "data", "countries.topo.json");
  const topo = JSON.parse(fs.readFileSync(topoPath, "utf8"));

  console.log("Enriching TopoJSON with ISO3 codes...");

  // Get the countries object (could be named differently)
  ...
  for (const geometry of geometries) {
    ...
    if (iso3) {
      ...
      geometry.properties.ISO_A3 = iso3;
      matched++;
    } else {
      console.log(`  No ISO3 mapping for numeric ID: ${geometry.id} (${geometry.properties?.name})`);
      unmatched++;
    }
  }

  // Write enriched file
  fs.writeFileSync(topoPath, JSON.stringify(topo), "utf8");

  console.log(`\n✅ Enrichment complete!`);
  console.log(`   Matched: ${matched} countries`);
  console.log(`   Unmatched: ${unmatched} countries`);
  console.log(`   Output: ${topoPath}`);
}

enrichTopojson().catch(console.error);
